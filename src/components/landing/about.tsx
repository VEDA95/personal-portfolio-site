    ...
    return (
        <Waypoint onEnter={handleEnter}>
            <section className="flex flex-col w-full min-h-[calc(100vh_-_7rem)] justify-center mt-32 px-6 md:px-8">
                <div className='flex flex-row w-full pt-16 pl-48'>
                    <animated.h1
                    ...
                    <article className="flex flex-col w-3/4 md:w-1/2 min-h-full justify-end">
                        <animated.div
                            ...
                            {content.map((item: ISectionBlock, index: number): ReactElement<FC> => {
                                if(index === 0) return <p key={`about-content-para-${index + 1}`}>{item.data.text}</p>;

                                return <p key={`about-content-para-${index + 1}`} className="pt-4">{item.data.text}</p>;
                            })}
                        </animated.div>
                    </article>
                </div>
